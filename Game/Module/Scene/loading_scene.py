...
class Loading(Scene):
    ...
    def create_loading_bar(self):
        ...
        self.load_position_bg = (self.width - self.loading_bg_width - (self.distance * 2),self.height- self.loading_bg_height - (self.distance * 2) )
        
        self.loading_bar_width = 516
        self.loading_bar_height = 90
        self.load_position_bar = (self.width - self.loading_bar_width - (self.distance * 2),self.height- self.loading_bar_height - (self.distance * 2) )
    ...
    def runningLoadingBar(self):
        if not self.loading_finished:
            ...
            scaled_loading_bar = transform.scale(self.loading_bar, (int(self.loading_bar_width),80))
        
        
            self._window.blit(self.bg_image, (0, 0))  # Redraw the full background
            self._window.blit(transform.scale(image.load("Game Assets\main_assets\logo.png"), (int(self.width*0.20), int(self.height*0.22))), (12,12))
            
            self._window.blit(self.loading_bg, self.load_position_bg)  # Redraw the loading bar background
