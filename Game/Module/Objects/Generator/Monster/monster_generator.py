...
class MonsterGenerator(Object):
    def __init__(
        self,
        window: Surface,
        images_url: dict[dict[list]],
        chunk_dimension: dict,
        monster_counts: int = 100,
    ):
        ...
        self.monsters: list[Monster] = self.generateMonsters(
            monster_counts=self.monster_counts
        )

    def generateMonster(self):
        monster_type = random.choice(list(self.images_url.keys()))

        diapason_after_window_dimension = [0, 0]
        x, y = [
            random.randint(self.chunk_dimension[0][0], 0),
            random.randint(self.chunk_dimension[1][0], 0),
        ]

        if self.window_size[0] < self.chunk_dimension[0][1]:
            diapason_after_window_dimension[0] = random.randint(
                self.window_size[0], self.chunk_dimension[0][1]
            )
        if self.window_size[1] < self.chunk_dimension[1][1]:
            diapason_after_window_dimension[1] = random.randint(
                self.window_size[1], self.chunk_dimension[1][1]
            )

        if diapason_after_window_dimension != [0, 0]:
            x = random.choice([x, diapason_after_window_dimension[0]])
            y = random.choice([y, diapason_after_window_dimension[1]])
        ...
        return Monster(
            ...
            stamina=self.images_url[monster_type]["stamina"],
        )
