...
class Character(Object):
    ...
    async def control(self, with_controller: bool = False):
        ...
        if time() < self.get_damage_max_delay / 2:
            if self.knockback_perspective == 0:
                self.pos[1] -= self.knockback_speed
            if self.knockback_perspective == 1:
                self.pos[0] += self.knockback_speed
            if self.knockback_perspective == 2:
                self.pos[1] += self.knockback_speed
            if self.knockback_perspective == 3:
                self.pos[0] -= self.knockback_speed
        ...
        if self.press_shift and not self.not_action and self.stamina > 0.1:
        ...
        if not self.press_shift and self.stamina < self.max_stamina:
            self.stamina += 0.004
        ...
        if (
            ...
            and time() >= self.attack_max_delay
            or (
                self.click_left_button
                and time() >= self.attack_max_delay
                and self.get_damage
            )
        ) and self.stamina >= 0.6:
            self.type_animation = (
                3
                if self.type_animation == 0
                else (
                    4
                    if self.type_animation == 1
                    else 5 if self.type_animation == 2 else self.type_animation
                )
            )
            self.attack_max_delay = time() + self.attack_delay
    ...
    async def checkCoincideTwoAxes(
        self, axe1_side1, axe1_side2, axe2_side1, axe2_side2
    ):
        return (
            (axe1_side1 <= axe2_side2 and axe1_side2 >= axe2_side2)
            or (axe1_side1 <= axe2_side1 and axe1_side2 >= axe2_side1)
            or (axe2_side1 <= axe1_side1 and axe1_side2 <= axe2_side2)
        )

    async def checkCoincideTwoPlaces(
        self,
        axe1_side1,
        axe1_side2,
        axe2_side1,
    ...
    async def damage(self, object: Object):
        if self.click_left_button and self.life != None and self.ctype != object.ctype:
            if self.perspective == 0:
                attack_range_x = (
                    self.centers[0] - self.attack_range[0] / 2,
                    self.centers[0] + self.attack_range[0] / 2,
                )
                attack_range_y = (
                    self.pos[1] - self.attack_range[1],
                ...
                attack_range_x = (
                    self.centers[0],
                    self.pos[0] + self.size[0] + self.attack_range[1],
                )
                attack_range_y = (
                    self.centers[1] - self.attack_range[0] / 2,
                    self.centers[1] + self.attack_range[0] / 2,
                )
                # pg.draw.rect(self.window, 'red', (attack_range_x[0], attack_range_y[0], attack_range_x[1]-attack_range_x[0], attack_range_y[1]-attack_range_y[0]))
                ...
                attack_range_y = (
                    self.pos[1] + self.size[1] / 2,
                    self.pos[1] + self.size[1] + self.attack_range[1],
                )
                # pg.draw.rect(self.window, 'red', (attack_range_x[0], attack_range_y[0], attack_range_x[1]-attack_range_x[0], attack_range_y[1]-attack_range_y[0]))
            ...
            if coincide_two_places:
                await object.giveDamage(
                    damage=self.damages, perspective=self.perspective
                )
